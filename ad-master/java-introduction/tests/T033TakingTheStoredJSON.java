...
public class T033TakingTheStoredJSON {
    ...
    public void testHomeCinemaPreferencesRetrievesUsername() {
        ...
        try {
            ...
            Class enumCls = Class.forName("HomeCinemaPreferencesMode");
            Object[] enumConstants = enumCls.getEnumConstants();
            Object jsonConstant = null;
            for (int i = 0; i < enumConstants.length; i++) {
                if (enumConstants[i].toString().endsWith("MODE_JSON")) {
                    jsonConstant = enumConstants[i];
                }
            }
            assert(jsonConstant != null);
            Object instance = cls.getDeclaredConstructor(enumCls).newInstance(jsonConstant);
            //Method initializeFromJson = cls.getDeclaredMethod("initializeFromJSON");
            //initializeFromJson.setAccessible(true);
            //initializeFromJson.invoke(instance);
