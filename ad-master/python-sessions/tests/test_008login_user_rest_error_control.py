...
class IdeAPILoginNotHappyPathTestCase(unittest.TestCase):
    ...
    def test_missing_param_password(self):
        def test_body():
            request_body_missing_param = {
                "user_email": "test_" + str(time.time()) + "@test.co.uk",
            }
    ...
    def test_user_not_found(self):
        def test_body():
            request_body = {
                "user_email": "test_" + str(time.time()) + "@test.co.uk", # Never registered before
                "password": "asdf12345"
            }
            self.__assert_post_endpoint("v1/sessions", request_body, {"error": "User not in database"}, 404)
