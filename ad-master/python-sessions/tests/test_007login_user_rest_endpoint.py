...
class IdeAPILoginHappyPathTestCase(unittest.TestCase):
    ...
    def test_unsupported(self):
        def test_body():
            self.__assert_get_endpoint("v1/sessions", {"error": "HTTP method not supported"}, 405)
    ...
    def test_success_response(self):
        ...
        command = 'sqlite3.exe apis/IdeAPI/db.sqlite3 "INSERT INTO idearest26app_customuser (e_mail, username, encrypted_password) VALUES (\'' + valid_email + '\', \'Test User\', \'' + hashed + '\')"'
    ...
    def test_session_added_to_db(self):
        ...
        # And now verify DB
        self.assertTrue(os.path.isdir(context.IDEAPI_ROOT + "idearest26app"))
        command = 'sqlite3.exe apis/IdeAPI/db.sqlite3 "SELECT COUNT(token) FROM idearest26app_usersession WHERE token=\'' + self.result_token + '\'"'
        output = subprocess.Popen(command, shell=True, cwd=context.PROJECT_ROOT, stdout=subprocess.PIPE).communicate()[0]
