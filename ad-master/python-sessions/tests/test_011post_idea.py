...
{
 "model": "idearest26app.customuser",
 "pk": 1,
 "fields": {
...
{
 "model": "idearest26app.usersession",
 "pk": 1,
 "fields": {
  "person": 1,
  "token": "''' + self.random_token + '''"
    ...
    def test_new_idea_unauthenticated_response(self):
        def test_body():
            expected = {"error": "Invalid authentication"}
            client_body = {
                "idea_name": "ATitle",
                "content": "AContent"
            }
    ...
    def test_missing_parameter_response(self):
        def test_body():
            expected = {"error": "Missing parameter in request"}
            client_body = {
                "idea_name": "ATitle",
            }
    ...
    def __assert_test_database_contains(self, string_sqlite_tuple):
        command = 'sqlite3.exe apis/IdeAPI/db.test.sqlite3 "SELECT id, title, description FROM idearest26app_idea"'
