...
class IdeAPIRegisterHappyPathTestCase(unittest.TestCase):
    ...
    def test_success_response(self):
        def test_body():
            ...
            self.__assert_post_endpoint("v1/users", request_body, {"is_created": True}, 201)
    ...
    def test_user_added_to_db(self):
        ...
        command = 'sqlite3.exe apis/IdeAPI/db.sqlite3 "SELECT COUNT(e_mail) FROM idearest26app_customuser WHERE e_mail=\'' + random_email + '\'"'
    ...
    def test_user_added_to_db_with_correctly_hashed_pw(self):
        ...
        # And now verify DB
        self.assertTrue(os.path.isdir(context.IDEAPI_ROOT + "idearest26app"))
        command = 'sqlite3.exe apis/IdeAPI/db.sqlite3 "SELECT encrypted_password FROM idearest26app_customuser WHERE e_mail=\'' + random_email + '\'"'
        output = subprocess.Popen(command, shell=True, cwd=context.PROJECT_ROOT, stdout=subprocess.PIPE).communicate()[0]
