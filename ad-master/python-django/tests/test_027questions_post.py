...
class DashboardsAPINewQuestionEndpointTestCase(unittest.TestCase):
    ...
    def setUp(self):
        ...
        file.write('''
                [
          {
            "model": "dashboard26app.dashboard",
            "pk": 1,
            "fields": {
              "title": "Forocoches",
              "description": "Test"
    ...
    def test_new_question_response(self):
        def test_body():
            client_body = {"title": "New Question", "description": "A Question"}
    ...
    def test_new_question_is_actually_adde1(self):
        def test_body():
            ...
            self.__assert_test_database_contains("SELECT id, title, description FROM dashboard26app_question WHERE dashboard_id=1", "|New Question|A Question")
        # Run server and run test
        self.__test_with_server_alive(test_body)

    def test_new_question_is_actually_added2(self):
        def test_body():
            client_body = {"title": "New Question2", "description": "A Question2"}
            # Send POST
            self.__assert_endpoint("api/v1/dashboards/2/questions", client_json=client_body, expected_status_code=201)
            # And assert content in DB
            self.__assert_test_database_contains("SELECT id, title, description FROM dashboard26app_question WHERE dashboard_id=2", "|New Question2|A Question2")
        # Run server and run test
        self.__test_with_server_alive(test_body)
    ...
    def __test_with_server_alive(self, block):
        ...
        def app_dir_exists():
            return os.path.isdir(context.DASHBOARD_API_ROOT + "dashboard26app")
