...
class WallAPINewCommentEndpointTestCase(unittest.TestCase):
    ...
    def setUp(self):
        ...
        file.write('[{"model": "wallrest26app.entry", "pk": 1, "fields": {"title": "Title1", "content": "Content1", "publication_date": "2022-07-13T10:58:27Z"}}, {"model": "wallrest26app.entry", "pk": 2, "fields": {"title": "Title2", "content": "Content2", "publication_date": "2022-07-13T11:04:50Z"}}]')
        file.close()
    ...
    def test_new_comment_is_actually_added(self):
        def test_body():
            ...
            self.__assert_test_database_contains("SELECT id, content FROM wallrest26app_comment", "|AContent2")
        # Run server and run test
        self.__test_with_server_alive(test_body)
    ...
    def test_new_comment_entry2_is_actually_added(self):
        def test_body():
            ...
            # And assert content in DB
            self.__assert_test_database_contains("SELECT id, content FROM wallrest26app_comment WHERE entry_id = 2", "|AContent3")
        # Run server and run test
        self.__test_with_server_alive(test_body)
    ...
    def __test_with_server_alive(self, block):
        ...
        def app_dir_exists():
            return os.path.isdir(context.WALL_API_ROOT + "wallrest26app")
