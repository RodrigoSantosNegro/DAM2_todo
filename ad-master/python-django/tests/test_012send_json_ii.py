...
class SimpleAPIPostFavoriteAnimalEndpointTestCase(unittest.TestCase):
    ...
    def test_animal_cat_response(self):
        def test_body():
            self.__assert_endpoint("favorite_animal", client_json={"name": "Cat"}, expected_json_content={"message": "That is great! Seven lives will be enough"}, expected_status_code=200)
        # Run server and run test
        self.__test_with_server_alive(test_body)
    ...
    def test_animal_other_response(self):
        def test_body():
            ...
            self.__assert_endpoint("favorite_animal", client_json={"name": r}, expected_json_content={"message": "OK! Have a wonderful day"}, expected_status_code=200)
        # Run server and run test
        self.__test_with_server_alive(test_body)
