...
public class T019JSONResponseOtherExample {
    ...
    public void testExampleEndpointCharacterDescription() {
        try {
            ...
            JSONObject result = getUrl("http://localhost:8126/example4");
            assert(result != null);
            assertEquals("Homer J. Simpson es un hombre de muchas agallas, pero con un punto débil: Las rosquillas", result.getString("characterDescription"));
    ...
    public void testExampleEndpointCharacterPopularity() {
        try {
            ...
            JSONObject result = getUrl("http://localhost:8126/example4");
            assert(result != null);
            assertEquals(12, result.getInt("characterPopularity"));
            this.stopServer.invoke(this.currentServerInstance);
        } catch (ClassNotFoundException e) {
            fail("La clase especificada no existe");
        } catch (NoSuchMethodException e) {
