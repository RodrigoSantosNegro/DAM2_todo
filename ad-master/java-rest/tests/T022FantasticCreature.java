...
public class T022FantasticCreature {
    ...
    public void testName() {
        try {
            ...
            JSONObject result = getUrl("http://localhost:8126/creature");
            assert(result != null);
            assertEquals("Dragón negro de ojos rojos", result.getString("name"));
    ...
    public void testLifespan() {
        try {
            ...
            JSONObject result = getUrl("http://localhost:8126/creature");
            assert(result != null);
            assertEquals(334, result.getInt("lifespanYears"));
            this.stopServer.invoke(this.currentServerInstance);
        } catch (ClassNotFoundException e) {
            fail("La clase especificada no existe");
        } catch (NoSuchMethodException e) {
    ...
    public void testAbility() {
        try {
            ...
            JSONObject magic = result.getJSONObject("magic");
            assert(magic != null);
            assertEquals("Se dice que su aliento ígneo alcanza la temperatura del Sol", magic.getString("ability"));
            this.stopServer.invoke(this.currentServerInstance);
        } catch (ClassNotFoundException e) {
            fail("La clase especificada no existe");
        } catch (NoSuchMethodException e) {
