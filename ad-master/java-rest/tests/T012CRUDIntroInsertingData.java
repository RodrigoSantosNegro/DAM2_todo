...
public class T012CRUDIntroInsertingData {
    ...
    public void checkNumberOfTitles() {
        ...
        try {
            ...
            ResultSet result = statement.executeQuery("SELECT COUNT(*) FROM TMovies WHERE title = 'Inside Out' AND synopsis = 'Riley es una chica que disfruta o padece toda clase de sentimientos. Aunque su vida ha estado marcada por la Alegría, también se ve afectada por otro tipo de emociones. Lo que Riley no entiende muy bien es por qué motivo tiene que existir la Tristeza en su vida. Una serie de acontecimientos hacen que Alegría y Tristeza se mezclen en una inesperada aventura que dará un vuelco al mundo de Riley.'");
            while(result.next()) {
