...
public class T014PutRequest extends TestRun {
    ...
    public static void setUp() throws ClassNotFoundException, IOException, InterruptedException, JSONException {
        ...
        usernameOkStatus = "Dev26UserOK" + (new Date()).getTime();
        passwordOkStatus = "abc";
        usernameKoStatus = "Dev26UserKO" + (new Date()).getTime();
    ...
    public void checkPutIsSent() throws JSONException, ClassNotFoundException, NoSuchMethodException {
        ...
        // Login
        Espresso.onView(withHint("william.doe")).perform(typeText(usernameOkStatus));
        ...
        Espresso.onView(editTextWithHint("Hey! I've changed my status")).perform(typeText(newStatus));
        Espresso.closeSoftKeyboard();
        Espresso.onView(buttonWithText("Nuevo estado")).perform(click());
        // Assert
    ...
    public void checkLoadingIsShown() throws JSONException, ClassNotFoundException, NoSuchMethodException {
        ...
        statusCls.getDeclaredMethod("sendUpdateStatusRequest");
        // Login
        Espresso.onView(withHint("william.doe")).perform(typeText(usernameKoStatus));
        Espresso.closeSoftKeyboard();
        Espresso.onView(withHint("Contraseña")).perform(typeText(passwordKoStatus));
        Espresso.closeSoftKeyboard();
        Espresso.onView(withText("Login")).perform(click());
